...
def run_loop(sim_name,
    ...
    tb.define_toolbox_paths(BorC2T_type  = BorC2T_type,
                            ispd_thresh  = ispd_thresh,
                            bin_win_days = bin_win_days,
                            bin_min_days = bin_min_days,
                            mean_period  = mean_period,
                            ice_type     = ice_type)
    P_zarr      = tb.P_zarrs[ice_type]['data']['raw']
    P_zarr_bin  = tb.P_zarrs[ice_type]['data']['bin']
    P_zarr_roll = tb.P_zarrs[ice_type]['data']['roll']
    ...
    for yr0_str, yrN_str in zip(tb.yr0_strs, tb.yrN_strs):
        ...
        for mo0_str, moN_str in zip(tb.mo0_strs, tb.moN_strs):
            I_raw, I_bin, I_roll, _ = tb.classify_ice(dt0_str=mo0_str, dtN_str=moN_str, enable_rolling_output=enable_rolling)
        ...
        I_bin_yr  = xr.concat(I_bin_mo, dim="time").chunk(tb.CICE_dict["FI_chunks"]) if I_bin_mo else None
        I_roll_yr = xr.concat(I_roll_mo, dim="time").chunk(tb.CICE_dict["FI_chunks"]) if I_roll_mo else None
        tb._write_grouped_zarr(I_yr, store=P_zarr, group=yr_str, overwrite_group=overwrite_zarr, consolidated=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ispd_thresh",
                        type=float,
                        default=None,
                        help="Ice speed threshold for masking fast ice")
    parser.add_argument("--BorC2T_type",
                        nargs="+",
                        choices=["B", "Ta", "Tb", "Tc", "Tx"],
                        help="One or more masking/regrid types")
    parser.add_argument("--ice_type", default="FI", help="Ice type: FI, PI, SI, MIZ")
    ...
    parser.add_argument("--overwrite_zarr", action="store_true", help="Overwrite existing output groups")
    parser.add_argument("--rolling_mean", action="store_true",
                        help="Enable rolling-mean output in sea ice classification")
    parser.add_argument("--delete_original_iceh",
                        action="store_true",
                        help="After creating grouped iceh zarr, delete original daily ice history files")
...
#         I_yr = tb._sanitize_for_zarr_write(I_yr, cast_float32=True)
#         I_yr = tb._strip_unsafe_zarr_encoding(I_yr)
#         I_yr.to_zarr(P_zarr,
#                      group        = yr_str,
#                      mode         = "w" if (not os.path.exists(P_zarr)) else "a",
#                      consolidated = False,
#                      zarr_format  = 2)
#         if I_bin_yr:
#             tb.logger.info(f"writing {P_zarr_bin}")
#             I_bin_yr = tb._strip_unsafe_zarr_encoding(I_bin_yr)
#             I_bin_yr.to_zarr(P_zarr_bin,
#                              group        = yr_str,
#                              mode         = "w" if (not os.path.exists(P_zarr_bin)) else "a",
#                              consolidated = False,
#                              zarr_format  = 2)
#         if I_roll_yr:
#             tb.logger.info(f"writing {P_zarr_roll}")
#             I_roll_yr = tb._strip_unsafe_zarr_encoding(I_roll_yr)
#             I_roll_yr.to_zarr(P_zarr_roll,
#                               group        = yr_str,
#                               mode         = "w" if (not os.path.exists(P_zarr_roll)) else "a",
#                               consolidated = False,
#                               zarr_format  = 2)
