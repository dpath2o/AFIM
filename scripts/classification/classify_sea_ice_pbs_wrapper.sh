...
print_help() {
  echo ""
  echo "Usage:"
  echo "  $0 -s SIM_NAME -t ISPD_THRESH [-i ICE_TYPE] [-g BorC2T_TYPE ...] [-S START_DATE] [-E END_DATE] [-x NC_ENG] [-J P_JSON] [-W BIN_WIN_DAYS] [-M BIN_MIN_DAYS] [-P MEAN_PERIOD] [-r] [-z] [-y] [-k] [-n]"
  echo ""
  echo "Options:"
  ...
  echo "  -r                 Enable rolling-mean classification"
  echo "  -z                 Overwrite zarr output"
...
# --- Parse options ---
while getopts "s:i:t:g:S:E:x:J:W:M:P:rzkynh" opt; do
  case "${opt}" in
    s) SIM_NAME="$OPTARG" ;;
    i) ICE_TYPE="$OPTARG" ;;
...
if [[ -z "$ISPD_THRESH" ]]; then
  echo "❌ Error: -t ISPD_THRESH is required." >&2
  print_help
  exit 1
fi
...
# Serialise BorC2T list safely through qsub -v
BorC2T_TYPE_SERIALISED="$(IFS=:; echo "${BorC2T_TYPE_LIST[*]}")"
...
while [[ "$current_period" < "$end_period" || "$current_period" == "$end_period" ]]; do
  ...
  VAR_PASS="SIM_NAME=${SIM_NAME},MONTH=${MONTH},YEAR=${YEAR},START_DATE=${DT0},END_DATE=${DTN},ICE_TYPE=${ICE_TYPE},ISPD_THRESH=${ISPD_THRESH},NC_ENG=${NC_ENG},P_JSON=${P_JSON},BorC2T_TYPE=${BorC2T_TYPE_SERIALISED}"
  ...
  [[ -n "$BIN_WIN_DAYS" ]]               && VAR_PASS+=",BIN_WIN_DAYS=${BIN_WIN_DAYS}"
  [[ -n "$BIN_MIN_DAYS" ]]               && VAR_PASS+=",BIN_MIN_DAYS=${BIN_MIN_DAYS}"
  [[ -n "$MEAN_PERIOD" ]]                && VAR_PASS+=",MEAN_PERIOD=${MEAN_PERIOD}"

  JOB_NAME="${SIM_NAME}_${ICE_TYPE}_${ISPD_THRESH}_BW${BIN_WIN_DAYS}_BM${BIN_MIN_DAYS}_${JOB_SUFFIX}"

  if [[ "$DRY_RUN" == true ]]; then
    echo "[DRY RUN] qsub -N ${JOB_NAME} -v \"${VAR_PASS}\" ${PBS_SCRIPT}"
  else
    qsub -N "${JOB_NAME}" -v "${VAR_PASS}" "${PBS_SCRIPT}"
  fi
