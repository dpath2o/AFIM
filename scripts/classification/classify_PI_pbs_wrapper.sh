...
print_help() {
    ...
    echo "  -i B2T_TYPE     B-grid regridding to T-grid method(s): B (no regridding), Ta, Tb, Tx, BT (some combination)"
...
while [[ "$current_period" < "$end_period" || "$current_period" == "$end_period" ]]; do
    ...
    JOB_NAME="pi_${SIM_NAME}_${JOB_SUFFIX}_${ISPD_THRESH}"
