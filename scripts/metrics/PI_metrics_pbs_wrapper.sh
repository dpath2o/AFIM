...
SIM_NAME=""
ISPD_THRESH="5e-4"
ICE_TYPE_LIST=("PI_B" "PI_Ta" "PI_Tx" "PI_BT")
...
print_help() {
    ...
    echo "  -i ICE_TYPE        Ice type(s): PI_B, PI_Ta, PI_Tx, PI_BT (repeatable or comma-separated)"
    ...
    echo "  -d DAYS            Days to smooth PIA time series (default: 15)"
    echo "  -n                 Dry run: print the qsub command without submitting"
    echo "  -h                 Show this help message and exit"
    echo ""
    echo "Examples:"
    echo "  $0 -s gi-mid -t 1e-3 -i PI_Ta -b -z -p"
    echo "  $0 -s gi-mid -t 1e-3 -i PI_B,PI_Tx -d 7"
    echo ""
}
...
JOB_NAME="pi_mets_${SIM_NAME}_${ISPD_THRESH}"
