...
def safe_compute_sea_ice_metrics(tb, I_dict, P_mets_zarr, overwrite=False):
    try:
        ...
        tb._write_grouped_zarr(DS_METS, store=P_mets_zarr, group="", overwrite_group=overwrite, consolidated=False)
...
def main(P_JSON, sim_name, ispd_thresh, ice_type, BorC2T_type,
    ...
    P_mets_raw = tb._resolve_product_store(ice_type     = ice_type,
                                           class_method = "raw",
                                           product      = "mets",
                                           BorC2T_type  = BorC2T_type,
                                           ispd_thresh  = ispd_thresh,
                                           ...
                                           bin_min_days = bin_min_days,
                                           mean_period  = mean_period)
    safe_compute_sea_ice_metrics(tb, I_dy, P_mets_raw, overwrite=overwrite_zarr)
    if ice_type != "SI":
        P_mets_bin = tb._resolve_product_store(ice_type     = ice_type,
                                               ...
                                               bin_min_days = bin_min_days,
                                               mean_period  = mean_period)
        safe_compute_sea_ice_metrics(tb, I_bn, P_mets_bin, overwrite=overwrite_zarr)
        if rolling_mean:
            P_mets_roll = tb._resolve_product_store(ice_type     = ice_type,
                                                    ...
                                                    bin_min_days = bin_min_days,
                                                    mean_period  = mean_period)
            safe_compute_sea_ice_metrics(tb, I_rl, P_mets_roll, overwrite=overwrite_zarr)
