...
def main(sim_name, ispd_thresh, ice_type, dt0_str, dtN_str, smooth_SIA_days, overwrite_zarr, overwrite_png):
    ...
    P_log     = Path(Path.home(), "logs", f"metrics_SI_{sim_name}_ispd_thresh{ispd_thresh}.log")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute SIA and related metrics, apply boolean mask, and plot spatial + temporal outputs.")
    parser.add_argument("--sim_name", type=str, required=True)
    parser.add_argument("--ispd_thresh", type=float, required=True)
    parser.add_argument("--ice_type", nargs="+", default=["SI_BT"], help="must be of the format [TYPE_OF_ICE]_[TYPE_OF_GRID] (e.g. 'SI_BT')")
    parser.add_argument("--start_date", default="1994-01-01", help="Start date (YYYY-MM-DD), which is then added to SI_days as the first center-date")
    parser.add_argument("--end_date", default="1999-12-31", help="End date (YYYY-MM-DD), will stop processing when this end_date-SI_days")
    parser.add_argument("--overwrite_zarr", action="store_true")
    parser.add_argument("--overwrite_png", action="store_true")
    parser.add_argument("--smooth_SIA_days", type=int, default=0)
