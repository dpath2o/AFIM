...
class GroundedIcebergProcessor:
    ...
    def load_ice_shelves(self):
        ...
        self.antarctic_ice_shelves = shelves
        self.ice_shelves_loaded = True

    def check_existing_files(self):
        """
        Check whether the expected output files already exist and raise an error
        unless `overwrite=True` was set in the constructor.

        This is a safeguard against overwriting KMT or GI-thinned NetCDFs that
        might have been used in a numerical simulation.
        """
        if os.path.exists(self.GI_dataset_path) or os.path.exists(self.KMT_path):
            if not self.overwrite:
                raise FileExistsError(
                    f"Thinned GI file or KMT file already exists:\n  - {self.GI_dataset_path}\n  - {self.KMT_path}\nThese files may have been used in model simulations. Set `overwrite=True` in the constructor to allow overwriting."
                )
    ...
    def load_existing_thinned(self):
        ...
        """
        if os.path.exists(self.GI_dataset_path):
            self.GI_thin_da = xr.open_dataarray(self.GI_dataset_path)
            self.GI_thin_mask = self.GI_thin_da.values > 0
        if os.path.exists(self.KMT_path):
            self.GI_KMT = xr.open_dataarray(self.KMT_path)
    ...
    def load_AFIM_GI(self):
        ...
        GI_locs             = xr.open_dataset(self.GI_dataset_path)
        G_pts               = np.column_stack((self.G_t["lon"].values.ravel(), self.G_t["lat"].values.ravel()))
        GI_pts              = np.column_stack((GI_locs["lon"].values, GI_locs["lat"].values))
        tree                = cKDTree(G_pts)
        ...
        KMT_mod             = xr.open_dataset(self.KMT_path)
        ...
        self.GI_lons        = GI_locs["lon"].values
        self.GI_lats        = GI_locs["lat"].values
    ...
    def load_grid_and_landmask(self):
        ...
        """
        grid_ds = xr.open_dataset(self.CICE_grid_path)
        kmt_ds = xr.open_dataset(self.KMT_path)
        G_t = xr.Dataset({
            'lat': (('nj', 'ni'), grid_ds.tlat.data * 180 / np.pi),
            'lon': (('nj', 'ni'), ((grid_ds.tlon.data * 180 / np.pi + 180) % 360) - 180),
            'area': (('nj', 'ni'), grid_ds.tarea.data)
        })
        G_u = xr.Dataset({
            'lat': (('nj', 'ni'), grid_ds.ulat.data * 180 / np.pi),
            'lon': (('nj', 'ni'), ((grid_ds.ulon.data * 180 / np.pi + 180) % 360) - 180),
            'area': (('nj', 'ni'), grid_ds.uarea.data)
        })
        KMT = kmt_ds.kmt
        if not self.use_gi and 'ny' in KMT.dims and 'nx' in KMT.dims:
            KMT = KMT.rename({'ny': 'nj', 'nx': 'ni'})
        self.G_t = G_t
        self.G_u = G_u
        self.KMT = KMT

    def process_raw_iceberg_csv(self):
        """
        ...
        """
        raw_path = self.GI_dict['P_raw']
        GI_raw = pd.read_csv(raw_path)
        ...
        nj, ni = self.G_t['lon'].shape
        nj_i, ni_i = np.unravel_index(nrst_i, (nj, ni))
        GI_clean['nj_index'] = nj_i
        GI_clean['ni_index'] = ni_i
        self.GI_clean = GI_clean

    def count_icebergs(self):
        """
        Count grounded icebergs per grid cell from cleaned data.

        Aggregates mapped iceberg coordinates to form a 2D count array.
        Sets `self.GI_cnts` as an xarray DataArray.
        """
        nj, ni = self.G_t['lon'].shape
        GI_cnts = np.zeros((nj, ni), dtype=int)
        GI_cnts_df = self.GI_clean.groupby(['nj_index', 'ni_index']).size().reset_index(name='count')
        GI_cnts[GI_cnts_df['nj_index'], GI_cnts_df['ni_index']] = GI_cnts_df['count']
        GI_cnts_da = xr.DataArray(GI_cnts, dims=self.spatial_dims, name='GI_counts')
        self.GI_cnts = GI_cnts_da

    def apply_thinning(self, p_min=0.1, p_max=0.9, scaling_exponent=1.5):
        """
        ...
        self.GI_thin_da = xr.DataArray(GI_thinned, dims=self.spatial_dims, name='GI_thinned_counts')
        self.GI_thin_mask = thin_mask
        self.GI_thin_fact = thin_fact

    def modify_kmt(self):
        """
        Modify the model KMT (landmask) by setting grounded iceberg cells to land (value=0).

        Sets `self.GI_KMT` as an xarray DataArray with the modified landmask.
        """
        kmt = self.KMT.values.copy()
        thin_nj, thin_ni = np.where(self.GI_thin_mask)
        ...
        GI_KMT = xr.DataArray(
            ...
            name='CICE_landmask_with_GI_as_land'
        )
        self.GI_KMT = GI_KMT

    def save_thinned_and_kmt(self):
        """
        ...
        P_thin = os.path.join(self.GI_dict['D_GI_thin'], F_thin)
        P_KMT_mod = os.path.join(self.GI_dict['D_GI_thin'], F_KMT_mod)
        self.GI_thin_da.to_netcdf(P_thin, mode='w')
        self.GI_KMT.to_netcdf(P_KMT_mod, mode='w')
