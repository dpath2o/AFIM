...
class SeaIcePlotter:
    ...
    def pygmt_base_layer(self, fig, region, projection,
                         title      = None,
                         frame      = "af",
                         coast      = True,
                         land       = "gray85",
                         water      = "white",
                         shorelines = "0.35p,black"):
        """
        Create a PyGMT figure with a basemap/coastline.
        """
        frame_use = [frame] if isinstance(frame, str) else list(frame)
        if title is not None:
            frame_use = frame_use + [f"+t{title}"]
        fig.basemap(region=region, projection=projection, frame=frame_use)
        if coast:
