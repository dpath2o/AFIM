...
! namelist input; initialize the atmospheric forcing data filenames.
      ...
      if (use_leap_years .and. (trim(atm_data_type) /= 'JRA55_gx1' .and. &
                                ...
                                trim(atm_data_type) /= 'era5' .and. & !dpath2o
  ...
  !dpath2o Add support for "era5" here
...
! that filenames end with 'yyyy.dat' or 'yyyy.r' or 'yyyy.nc'.
      ...
      elseif (trim(atm_data_type) == 'JRA55_tx1') then ! netcdf
         ...
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
      !dpath2o
      elseif (trim(atm_data_type) == 'era5') then ! netcdf
         i = index(data_file,'.nc') - 5
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
...
!dpath2o
    ...
    !assumptions:
    ! era5_for_cice6_YYYY.nc is the naming convention for the era5 data files, where YYYY is the year.
    ! atm_data_dir is a global variable that stores the path to the directory where your atmospheric data is stored.
    ! The function file_year() is used to update the file year; adjust this as necessary.
    ! I've added a helper function year_to_str to convert the year to a string.
    ! ! 
          integer (kind=int_kind), intent(in) :: &
               yr                   ! current forcing year
          ...
          ! Set the file name for era5 data; adjust directory and naming as needed
          uwind_file = &
               trim(atm_data_dir)//'/24XDAILY/era5_for_cice6_'//trim(adjustl(year_to_str(yr)))//'.nc'
    
          ! File year updating might be different for your case, 
          ...
          ! Convert year to string; adjust as needed
          function year_to_str(yr) result(yr_str)
            integer (kind=int_kind), intent(in) :: yr
            character(len=4) :: yr_str
    
            write(yr_str,'(I4)') yr
          end function year_to_str
...
subroutine ERA5_data
    !Major changes:
    ! sec1hr replaces sec3hr to account for hourly data.
    ! maxrec is now set to days_per_year * 24 to handle 24 hourly records per day.
    ! The recnum calculation has been changed to reflect the hourly timestep.
    ! fieldname assignments remain the same, assuming the variable names in the 
    ! ERA5 data are identical to JRA55.
    use ice_blocks, only: block, get_block
    use ice_global_reductions, only: global_minval, global_maxval
    ...
    ! ttlpcp   (precipitation kg/m s-1), hr average
    !-------------------------------------------------------------------
    ...
    do n1 = 1, 2
        ...
        elseif (n1 == 2) then
            recnum = 24*int(yday) - 23 + int(real(msec,kind=dbl_kind)/sec1hr) + 1
            if (recnum > maxrec) then
                lfyear = fyear + 1  ! next year
                if (lfyear > fyear_final) then
                    lfyear = fyear_init 
                    recnum = 1
                endif
                call file_year(uwind_file,lfyear)
    ...
    ! Compute interpolation coefficients
    eps = 1.0e-6
    ! Compute interpolation coefficients for hourly data
    ...
    call interpolate_data (vatm_data, vatm)
    call interpolate_data (Qa_data, Qa)
    ! use hrly average for heat flux and precip fields, no interpolation
    !      call interpolate_data (fsw_data, fsw)
    ...
    if (debug_forcing .or. local_debug) then
        if (my_task.eq.master_task) write (nu_diag,*) subname,'fdbg ERA5_bulk_data'
        vmin = global_minval(fsw,distrb_info,tmask)
        vmax = global_maxval(fsw,distrb_info,tmask)
